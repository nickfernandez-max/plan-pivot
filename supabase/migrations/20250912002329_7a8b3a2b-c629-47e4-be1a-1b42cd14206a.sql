-- Get Processing team ID and role IDs first (these are assumed based on existing data structure)
...
-- Create team memberships for the new members
INSERT INTO team_memberships (team_member_id, team_id, start_month) 
SELECT tm.id, tm.team_id, DATE_TRUNC('month', tm.start_date)::date
...
bas AS (
  SELECT tm.id, ROW_NUMBER() OVER (ORDER BY tm.name) as ba_num
  FROM team_members tm
...
devs AS (
  SELECT tm.id, ROW_NUMBER() OVER (ORDER BY tm.name) as dev_num  
  FROM team_members tm
  JOIN roles r ON tm.role_id = r.id
  WHERE tm.team_id = 'fbd5c124-a31d-446d-889c-e12e71567448' 
  AND r.name LIKE 'Dev%'
...
JOIN bas ON bas.ba_num = ((pp.project_num - 1) % (SELECT COUNT(*) FROM bas)) + 1
...
FROM processing_projects pp
JOIN random_allocations ra ON pp.project_num = ra.project_num
JOIN devs ON devs.dev_num = ((pp.project_num - 1) % (SELECT COUNT(*) FROM devs)) + 1;